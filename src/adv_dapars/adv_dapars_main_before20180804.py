...
#         print PDUI_cutoff
    ...
    for line in open(input_file,'r'):
        if num_line > 0:
            ...
            result_dict.append([fields[0],fields])
    ...
    for ele in result_dict:
        ...
        if curr_event_id in Selected_events_id:
            ...
            if curr_FDR_val=='NA' or curr_fields[-1]=='NA':
                Pass_filter = 'N'
            elif float(curr_FDR_val) <= FDR_cutoff and abs(float(curr_fields[-1]))>=PDUI_cutoff and abs(math.log((mean_PDUI_group1+1e-5)/(mean_PDUI_group2+1e-5),2))>=Fold_change_cutoff:
                Pass_filter = 'Y'
